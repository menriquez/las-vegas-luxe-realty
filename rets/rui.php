...
$base_photo_jpeg_dir = $current_root_dir . 'photos/jpeg/';
...
function begin_image_update($rets_object, $rets_name, $rets_config) {
	...
	sqft_living, sqft_tot, halfbaths, full_bath , bedrooms , year_built , active_DOM FROM master_rets_table 
...
function get_images($listing_id, $photo_count, $rets_key, $rets_object, $r, $rets_config): bool
{
    ...
    if ($GLOBALS['forceDl']) {
        ...
        foreach ($photos as $photo) {
	        ...
	        // write a jpeg of the front pic to the jpeg cache dir
	        if ($photo['Object-ID']=="0") {

		        $raw_jpeg_fn = build_seo_filenames_jpeg($r,$photo['Object-ID']);
		        $jpeg_fn = $base_photo_jpeg_dir . $raw_jpeg_fn;

		        if (imagejpeg($jpeg_res,$jpeg_fn,70)==false) {
			        echo "Could not write the file [ " . $jpeg_fn . " ] Skipping...".PHP_EOL;
		        } else {
			        echo "Writing BASE image [ " . $jpeg_fn . " ] Ok" . PHP_EOL;
		        }
	        }
        }
    }
...
function fixDBPhotoRecs()
{
    ...
    while ($row = mysqli_fetch_assoc($mls_r)) {
        ...
        if ($dir_image_count < $row['photo_count']) {
            ...
            $update_r = mysqli_query($conn, "UPDATE `master_rets_table` SET `photo_update` = '1' WHERE `listing_id` = '{$row['listing_id']}' AND `rets_system` = '{$row['rets_system']}'");
            $eq++;
        } else if ($dir_image_count > $row['photo_count']) {
            echo "Listing: {$row['listing_id']} from {$row['rets_system']} - - current image count [$dir_image_count] > database image count [$row[photo_count]]...???<br>" . PHP_EOL;
            $mr++;
...
function updateMasterRets($listing_id)
{
    ...
    $sql = "UPDATE master_rets_table set index_photo = 1 where listing_id = '$listing_id'";
...
function update_image_rec($mls_id) {
	...
	$sql = "UPDATE master_rets_table SET photo_update = 1 WHERE listing_id = $mls_id";
