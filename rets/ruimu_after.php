...
function begin_image_update($rets_object, $rets_name, $rets_config) {
	...
	sqft_living, sqft_tot, halfbaths, full_bath , bedrooms , year_built , active_DOM FROM master_rets_table_update 
                WHERE photo_timestamp  > (select photo_timestamp from master_rets_table_update where listing_id =  (select end_time_mlsid from photo_dl_info where end_time_mlsid <> '' order by id DESC limit 1)) 
                AND photo_count > 0
    ...
    echo "Ending master_rets_table_update for $rets_name image download [$totRows DLed]\n";
} //end begin_rets()
