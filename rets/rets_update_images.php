...
date_default_timezone_set("America/Los_Angelas");
...
$base_photo_image_dir = __DIR__ . '/photos/';
$base_hires_image_dir = __DIR__ . '/hires/';
$base_thumbs_image_dir = __DIR__ . '/thumb/';
...
// PHPRETS
include __DIR__ .'/phrets2.php';
$rets = new phRETS();
// set initial paramerts

$rets->SetParam('disable_follow_location', true);
fopen( "/home/admin/web/$ACTIVE_DOMAIN/public_html/rets/cookie.txt","w");

$rets->SetParam('cookie_file', '/home/admin/web/$ACTIVE_DOMAIN/public_html/rets/cookie.txt');
exec("chmod 777 /home/admin/web/$active_uri/public_html/rets/cookie.txt");
...
function begin_rets_image_update($rets_object, $rets_name, $rets_config) {
	...
	// also..IMPORTANT...this pulls only the records that have photos newer then the last pull  
	...
	if ($totRows > 0) {
		...
		while ($row = mysql_fetch_assoc($rets_results)) {
			...
			if ($first) {
				$end_time_db_ts = $row['photo_timestamp'];
				$first = false;
			}
		...
		$start_time_db_ts = $row['photo_timestamp'];

		// update timestamp only if we processed some records
		$sql= "UPDATE photo_dl_info SET ( 
						end_time_db_ts = '$end_time_db_ts', 
						end_time = NOW(), 
						start_time_db_ts = '$start_time_db_ts'
						) 
						WHERE id = $timeRecId";
...
function makeTable(){
	...
	if (mysql_num_rows($rowCnt)==0) {
		$initTbl=mysql_query("INSERT INTO photo_dl_info set start_time = '1900-01-01', end_time_db_ts = '1900-01-01';");
	}
...
function stampThumb($mls,$width) {
  ...
  execInBackground("php create_thumbs_mrtu.php $mls");
}
